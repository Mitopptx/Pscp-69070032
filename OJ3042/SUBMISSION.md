...
```text
finding all integers from N down to 0 that are divisible by 10, and display those numbers on the screen.
```
...
```text
Step 1: Input the number and use a while loop to continue as long as the number is not less than 0.
Step 2: Use an if-else statement to check whether the number is divisible by 10. If it is, print the number and subtract 10. Otherwise, subtract 1.
```
...
```text
I used this method and had some problems, but I solved them by myself.
```
...
```text
test start number using negative number
```
